...
when_creating_a StandardProcedure::Group, auto_generate: [:name], generate: { account: -> { a_saved StandardProcedure::Account } }
...
def a_saved_contact_called(name, reference: nil, account: nil, group: nil, role: nil, user: nil)
  ...
  return a_saved StandardProcedure::Contact, name: name, reference: reference, group: group, user: user, role: role
end
